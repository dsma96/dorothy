...
public interface VerifyRequestRepository extends JpaRepository<VerifyRequest, Integer> {
    @Query(nativeQuery = true,value= "SELECT * from verify_request v WHERE v.phone_no = :phoneNo and v.verify_type = :verifyType and v.verify_state = 'CREATED' and v.expire_date > now() order by v.expire_date DESC LIMIT 1 ")
    ...
    @Query(nativeQuery = true,value= "SELECT * from verify_request v WHERE v.phone_no = :phoneNo and v.verify_type = :verifyType and v.verify_state = :verifyState and v.verify_date > DATE_SUB( NOW(), INTERVAL 24 HOUR) order by v.expire_date DESC LIMIT 1 ")
